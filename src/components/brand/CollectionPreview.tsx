            ...
            <div className="max-w-[1920px] mx-auto">
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-16 gap-12 border-b border-white/5 pb-10">
                    <div className="space-y-4">
                        <span className="text-white/20 font-mono text-[10px] uppercase tracking-[0.6em]">New Collection / 01</span>
                    ...
                    <Link href="/shop" className="group flex items-center gap-6">
                        <span className="uppercase tracking-[0.4em] text-[10px] font-bold text-white group-hover:text-neutral-400 transition-colors">Archive Access</span>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                    {finalItems.map((item: any, i: number) => {
                        return <ProductCard key={item.id} item={item} index={i} formatAddonPrice={formatAddonPrice} />;
                    })}
                </div>
            </div>
        </section>
...
function ProductCard({ item, index, formatAddonPrice }: { item: any; index: number; formatAddonPrice: any }) {
    ...
    const priceRaw = item.price ? item.price.replace(/[^0-9.]/g, '') : "0";
    const priceDisplay = formatAddonPrice(parseFloat(priceRaw) || 0);
        ...
        <Link href="/shop" className="group block">
            ...
            <div className="flex justify-between items-start px-4">
                <div className="space-y-2">
                    ...
                    <p className="text-neutral-600 text-[9px] font-mono uppercase tracking-[0.4em]">Core Tech / Season 01</p>
                </div>
                {parseFloat(priceRaw) > 0 && (
                    <div className="bg-white/5 px-4 py-2 rounded-xl backdrop-blur-md border border-white/5 group-hover:bg-white group-hover:text-black transition-all">
                        <span className="text-white group-hover:text-black font-mono text-[10px] font-bold uppercase tracking-widest">{priceDisplay}</span>
                    </div>
                )}
            </div>
        </Link>
