...
export default function SingleProductPage() {
    ...
    // Price Display
    // Use WooCommerce's pre-formatted price string (already has correct currency symbol from Curcy)
    const rawPriceString = selectedVariation?.price || product?.price || "";
    ...
    // Note: This assumes standard formatting. If INR uses commas, we might need slightly better parsing, but usually WPGraphQL returns standard string.
    const basePriceNumeric = parseFloat(rawPriceString.replace(/[^0-9.]/g, '')) || 0;
    ...
    const totalPriceDisplay = formatValue(totalPriceNumeric);

    // Gallery Logic
    const mainImage = selectedVariation?.image?.sourceUrl || product.image?.sourceUrl || "";

    // Manual Variation Galleries (Frontend Override)
    const VARIATION_GALLERIES: Record<string, string[]> = {
        "blue": [
            "https://drotes.com/wp-content/uploads/2025/11/4V6A1899-scaled.jpg",
            "https://drotes.com/wp-content/uploads/2025/11/4V6A1898-scaled.jpg",
            "https://drotes.com/wp-content/uploads/2025/11/4V6A1895-scaled.jpg",
            "https://drotes.com/wp-content/uploads/2025/11/4V6A1881-scaled.jpg",
    ...
    // Override if custom gallery exists for color
    if (selectedColor) {
                    ...
                    <div className="flex flex-col animate-in fade-in slide-in-from-bottom-4 duration-700">
                        ...
                        <div className="flex gap-4 items-stretch h-14 mb-12">
                            ...
                            >
                                ...
                                {isAdding ? "Adding..." : (
                                    <>
                                        {isPreOrder ? "Pre-Order" : "Add to Bag"} &nbsp;&middot;&nbsp; {totalPriceDisplay}
                                    </>
                                )}
                            </button>
                        </div>
